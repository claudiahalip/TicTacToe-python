...
class TestsHumanPlayer:
    ...
    def test_mark_position_when_the_input_is_valid_and_the_position_is_not_taken(
        self, mock_input, mock_move, player_object
    ):
        player_object.mark_position()
        mock_move.assert_called_once()

    # # True, false
    @patch.object(UI, "get_user_input", side_effect=["4", "2"])
    @patch.object(UI, "display")
    def test_the_mark_position_when_the_input_is_valid_and_the_position_is_taken(
